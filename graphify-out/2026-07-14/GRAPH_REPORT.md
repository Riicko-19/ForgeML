...
## Corpus Check
- 116 files · ~46,937 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1235 nodes · 2036 edges · 93 communities (72 shown, 21 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 311 edges (avg confidence: 0.74)
...
2. `UnitOfWork` - 44 edges
3. `PackageLimits` - 35 edges
4. `AuditEvent` - 26 edges
5. `ZipArchiveReader` - 26 edges
6. `AppError` - 25 edges
7. `build_forge()` - 24 edges
8. `ErrorDetail` - 23 edges
...
- `_database_url()` --calls--> `load_settings()`  [INFERRED]
  backend/migrations/env.py → backend/src/forgeml/core/config.py
- `UnitOfWork` --uses--> `AuditLog`  [INFERRED]
  backend/src/forgeml/application/unit_of_work.py → backend/src/forgeml/domain/audit/ports.py
...
Cohesion: 0.13
Nodes (19): create_application(), FastAPI, FastAPI application composition root., Create an isolated Module 0 application without provider side effects., Frozen HTTP wire contract tests., test_framework_error_wire_shape_omits_empty_details(), test_health_wire_shapes_and_header(), Health endpoint integration tests. (+11 more)
...
Cohesion: 0.08
Nodes (31): ASGIApp, Server-owned request correlation and bounded request logging., Own the canonical request ID for one HTTP request., RequestContextMiddleware, current_request_id(), new_request_id(), Request-local server correlation context., Create a canonical server-owned request identifier. (+23 more)

### Community 3 - "ConfigurationFailure"
Cohesion: 0.19
Nodes (19): ConfigurationFailure, Exception, Resolve and validate the installed ForgeML distribution version., Fail-closed configuration error safe to classify at bootstrap., resolve_service_version(), MonkeyPatch, Configuration contract tests., test_explicit_valid_service_identity_is_accepted() (+11 more)

### Community 4 - "JsonEventFormatter"
Cohesion: 0.23
Nodes (14): JsonEventFormatter, Render a strict allowlisted JSON event., BaseException, LogRecord, TracebackType, Structured logging policy tests., _record(), test_event_and_fields_are_bounded() (+6 more)

### Community 5 - "AppError"
Cohesion: 0.06
Nodes (70): ErrorDetail, A bounded, safe detail for an expected application error., ArchiveEntry, ArchiveInspection, AssetSpec, EntrypointSection, is_supported_schema_dialect(), ManifestV1 (+62 more)
...
Cohesion: 0.05
Nodes (71): PackageValidationService, Validate a stored .forge artifact against the format version 1 contract., Runs archive validation over a stored artifact.      Asset checksums are the onl, PackageLimits, Operator policy bounding work spent on an untrusted .forge archive.      Every b, ArchiveUnreadable, Finding, Exception (+63 more)

### Community 8 - "session_factory"
Cohesion: 0.05
Nodes (55): create_database_engine(), create_session_factory(), Engine, Session, sessionmaker, Engine and session factory for the metadata database (ADR-009)., Build the metadata engine from typed settings.      `pool_pre_ping` costs one ro, Session factory for the unit of work.      `expire_on_commit=False` is deliberat (+47 more)

### Community 9 - "test_dependency_direction.py"
Cohesion: 0.32
Nodes (12): _imports(), AST-enforced dependency direction., The ORM is confined to one package.      If SQLAlchemy can be imported anywhere, test_api_never_imports_bootstrap_or_future_modules(), test_application_depends_on_domain_not_providers(), test_bootstrap_imports_core_not_api(), test_domain_depends_on_no_provider_transport_or_filesystem(), test_no_package_path_can_import_or_deserialize_package_content() (+4 more)
...
Cohesion: 0.12
Nodes (20): PackageState, StrEnum, Persisted package lifecycle (docs 04).      ValidationState is the validator's v, Terminal outcome of validating one archive., ValidationState, finding_from_json(), finding_to_json(), manifest_to_json() (+12 more)
...
Cohesion: 0.15
Nodes (12): ActorType, AuditEvent, StrEnum, Audit events (docs 04: append-only; no payloads, no secrets)., Who caused a state change., One append-only record of a state change.      Metadata is bounded and redacted, _safe_text(), to_audit_event() (+4 more)
...
Cohesion: 0.18
Nodes (12): PackageRow, PackageValidationRow, _conflict(), _decode_cursor(), _encode_cursor(), _not_found(), Any, datetime (+4 more)
...
Cohesion: 0.16
Nodes (17): FakeServer, BaseException, LogCaptureFixture, MonkeyPatch, Bootstrap and composition integration tests., test_configuration_failure_is_safe_and_exits_two(), test_logging_conflict_exits_one(), test_module_entrypoint_returns_bootstrap_exit_code() (+9 more)

### Community 53 - "test_openapi_contract.py"
Cohesion: 0.07
Nodes (35): AppError, ErrorCategory, Exception, StrEnum, Provider-neutral application error contracts., Transport-neutral classes of expected application failure., An immutable expected application failure., _validate_code() (+27 more)

### Community 62 - "OperationStore"
Cohesion: 0.18
Nodes (9): OperationStore, Any, Protocol, UUID, The durable operation store (ADR-006, ADR-010, ADR-016)., Durable operations, claimed by one worker at a time.      This store is the queu, Terminally succeed a running operation., Terminally fail a running operation. (+1 more)
...
Cohesion: 0.17
Nodes (9): AppError, _Clock, InMemoryOperationStore, InMemoryPackageCatalog, Any, datetime, UUID, In-memory implementations of the Module 2 ports.  Module 3 will test its use cas (+1 more)

### Community 68 - "AppSettings"
Cohesion: 0.17
Nodes (14): AppSettings, Immutable settings consumed by composition and bootstrap., In-process OpenAPI contract tests., test_openapi_contains_only_health_paths(), test_response_schemas_forbid_extra_fields(), _client_with_failure_routes(), Payload, BaseModel (+6 more)
...
Cohesion: 0.06
Nodes (28): Package, A stored package. Checksum and artifact are immutable (ADR-003).      manifest_v, ArchiveReader, ArtifactStore, PackageCatalog, PackagePage, Protocol, UUID (+20 more)
...
Cohesion: 0.18
Nodes (12): load_settings(), Load settings from an explicit mapping or the process environment., test_explicit_mapping_is_not_supplemented_by_process_environment(), test_non_string_explicit_value_is_validated_without_string_operations(), installed_version(), MonkeyPatch, Package limits are operator policy loaded through the fail-closed loader., test_an_invalid_limit_fails_closed() (+4 more)
...
Cohesion: 0.22
Nodes (7): Base, OperationRow, SQLAlchemy mappings. The only ORM classes in ForgeML.  Nothing here leaves this, Durable operations, claimed with row locking (ADR-010, ADR-016)., Reclaim work abandoned by a dead worker (ADR-016).          ADR-010 supervises e, SqlAlchemyOperationStore, DeclarativeBase
...
Cohesion: 0.29
Nodes (5): ConfigurationIssue, The metadata database URL, or a fail-closed configuration error.          Module, A safe configuration finding without an input value., _safe_issues(), ValidationError

## Knowledge Gaps
- **279 isolated node(s):** `ADR-001 — Trusted packages; defense-in-depth runtime isolation`, `ADR-002 — Modular monolith control plane`, `ADR-003 — Immutable content-addressed packages/images`, `ADR-004 — Metadata desired state; Docker reconciliation`, `ADR-005 — One active version and platform route` (+274 more)
...
- **Why does `AppSettings` connect `AppSettings` to `AppSettings`, `error_handlers.py`, `config.py`, `ConfigurationFailure`, `JsonEventFormatter`, `test_request_logging.py`, `session_factory`, `_EnvironmentSettings`, `test_package_limits.py`, `_reject_wildcard_host`, `test_application_boot.py`, `configure_logging`, `ConfigurationIssue`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `PackageLimits` connect `_run_handler` to `config.py`, `AppError`, `PackageCatalog`, `test_package_limits.py`, `test_openapi_contract.py`?**
  _High betweenness centrality (0.097) - this node is a cross-community bridge._
- **Why does `AuditEvent` connect `AuditEvent` to `create_application`, `Operation`, `ArtifactStore`, `event`, `models.py`, `UnitOfWork`, `SqlAlchemyOperationStore`?**
...
- **Are the 15 inferred relationships involving `PackageLimits` (e.g. with `PackageValidationService` and `ArchiveReader`) actually correct?**
  _`PackageLimits` has 15 INFERRED edges - model-reasoned connections that need verification._
- **Are the 12 inferred relationships involving `AuditEvent` (e.g. with `AuditLog` and `SqlAlchemyAuditLog`) actually correct?**
  _`AuditEvent` has 12 INFERRED edges - model-reasoned connections that need verification._